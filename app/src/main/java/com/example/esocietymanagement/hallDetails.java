...
public class hallDetails extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
            ...
            public void onClick(View v) {
                ...
                if(!(expTitle.getEditText().getText().toString().matches("") && yearS==null)){
                    if(!(expAmt.getEditText().getText().toString().matches(""))) {
                        ...
                        hallHelper hallHelper = new hallHelper(name, dec, yearS, months, dayS, amt);
                        dataexp.child("hallbook").child(yearS).child(months).child(dayS).setValue(hallHelper);

                        clr c1=new clr();
                        c1.clear(expAmt,expTitle,expDec);
                        expDec.clearFocus();

                        Toast.makeText(hallDetails.this, "Expense Added Successfully", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(hallDetails.this, "Fill All Details", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(hallDetails.this, "Fill All Details", Toast.LENGTH_SHORT).show();
                }
